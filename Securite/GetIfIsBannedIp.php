...
$nb_echecs = $securityManager->getIfBannedIp($ip);

file_put_contents("../.log", print_r($nb_echecs, true) . "\n", FILE_APPEND);

$isBanned = intval($nb_echecs[0]["succes"]) > 0;

if ($isBanned) {
    $securityManager->clearEchecCompteur($ip);
}


echo json_encode($isBanned);
